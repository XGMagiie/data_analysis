...
def profile_dataframe(df: pd.DataFrame, type_info: dict[str, dict[str, Any]], config: dict[str, Any]) -> dict[str, Any]:
    ...
    for col in df.columns:
        ...
        columns.append({"name": col, "semantic_type": t, "type_confidence": type_info[col].get("confidence"), "type_reason": type_info[col].get("reason"), "flags": flags, "statistics": stats})
        for flag in flags:
            quality_flags.append({"column": col, "flag": flag})
